-- Enables Paysera as an order choice while preserving the supplier-first workflow.
-- Customers still cannot pay until the administrator confirms stock.
BEGIN;

ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS payment_status text NOT NULL DEFAULT 'unpaid';
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS paysera_payment_url text;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS paysera_payment_created_at timestamptz;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS paysera_callback_received_at timestamptz;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS paysera_callback_status text;

UPDATE public.orders
SET payment_status = CASE WHEN paid_at IS NOT NULL THEN 'paid' ELSE 'unpaid' END
...
BEGIN
  IF NEW.payment_method NOT IN ('cod', 'paysera') THEN
    RAISE EXCEPTION 'Zgjidhni një mënyrë të vlefshme pagese.';
  END IF;
  IF NEW.supplier_status IS DISTINCT FROM 'pending_supplier_check'
    ...
    OR NEW.supplier_notes IS NOT NULL
    OR coalesce(NEW.payment_status, 'unpaid') IS DISTINCT FROM 'unpaid'
    OR NEW.paysera_payment_url IS NOT NULL
    OR NEW.paysera_payment_created_at IS NOT NULL
    OR NEW.paysera_callback_received_at IS NOT NULL
    OR NEW.paysera_callback_status IS NOT NULL THEN
  ...
  NEW.supplier_status := 'pending_supplier_check';
  NEW.payment_status := 'unpaid';
  NEW.paysera_payment_url := NULL;
  NEW.paysera_payment_created_at := NULL;
  NEW.paysera_callback_received_at := NULL;
  NEW.paysera_callback_status := NULL;
  NEW.created_at := now();
  RETURN NEW;
END;
$$;
